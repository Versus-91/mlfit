...
export default class LinearRegression extends ClassificationModel {
    ...
    async train(x_train, y_train, x_test, y_test, labels, categorical_columns) {
        ...
        const plotlyData = await webR.evalR(`
                    ...
                    base_model = cv.glmnet(as.matrix(scale_df), y, family = "multinomial", type.multinomial = "ungrouped")
                    weights <- 1 / abs(coef(base_model)[-1])
                    if(is_lasso){
                        cvfit = cv.glmnet(as.matrix(scale_df), y, alpha = 1,penalty.factor = weights, family = "multinomial", type.multinomial = "ungrouped")
                    }else{
